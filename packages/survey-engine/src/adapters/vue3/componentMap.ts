// adapters/vue3/componentMap.ts —— 物理迁移自 src/configs/componentMap.ts（T013，仅调整 import 路径）。
// 保留原有 componentMap 映射表对象导出（对外调用方式不变），同时在模块加载时逐条调用
// registerVue3Component() 将全部条目注册进 Vue3 组件工厂，供新的 resolveVue3Component() 查找方式使用。
// 业务组件
...
import { registerVue3Component } from "./componentFactory";
...
// 模块加载时逐条注册进 Vue3 组件工厂（markRaw 对已标记对象幂等，不会重复处理）
for (const [name, component] of Object.entries(componentMap)) {
  registerVue3Component(name, component);
}
