...
// 动态表单规则配置类型定义在 core/logic（框架无关纯逻辑层），此处按内部相对路径引用
import type { QuestionLogicConfig } from "../core/logic/index.js";

// 导出 vue 组件类型
export type VueComType = ReturnType<typeof defineComponent>;

/**
 * 后端统一响应信封（对应 Constitution Principle III）
 * @template T data 字段的类型，失败时为 null
 */
