<template>
  ...
  >
    ...
    <div class="radio-group">
      <!-- 选项联动处于"需先完成依赖题"提示态时，不展示任何选项，转而展示引导文案 -->
      <el-text v-if="isPoolPrompting" type="info" size="small">{{ t("components.optionDependencyPrompt") }}</el-text>
      <el-radio-group v-else v-model="radioValue" @click.stop @change="emitAnswer">
        <!--
          注意：radio 的 :value 始终取自完整选项数组 optionTexts 的原始下标 index，
          因为底层答案以该下标存储；候选池收窄只能通过 v-show 隐藏元素，不能过滤数组本身，
          否则会导致下标错位、答案与展示文本不一致
        -->
...
const props = defineProps<{
  ...
  /** 选项联动候选池：数组表示限定可选文本集合，{ prompt: true } 表示需先完成依赖题，undefined 表示未启用选项联动 */
 ...
 * 此处显式收窄以安全支持后续的选项联动过滤，非字符串数组时兜底为空数组
 */
