...
//
// 迁移说明（T020，原路径 src/logic/types.ts）：迁移至 core/logic/ 后，Material 的
// 导入来源由 "../types/material.js" 改为 "../schema/types"，切断到 vue 的 type-only
// 依赖链（原 types/material.ts 间接依赖 VueComType → vue），使本文件符合
// core/ 边界"不出现任何框架 import（含 type-only import）"的约束（research.md R1）
// ──────────────────────────────────────────────────────────────────────────────
