...
- [x] T030 推送到 `preview` 分支（或手动触发 `workflow_dispatch`），验证 GitHub Actions `Deploy q-editor to GitHub Pages` 工作流完整跑通并成功部署——过程中额外发现并修复 3 个真实问题：① `pnpm-lock.yaml` 被 `.gitignore` 排除导致 `actions/setup-node` 找不到锁文件；② `monorepo-tracking-sdk` 的 `package.json` 通过 `dist/index.js` 暴露产物但 CI 从未构建该包，导致 Rollup 无法解析该依赖，已在工作流中新增 `pnpm --filter monorepo-tracking-sdk build` 步骤；③ CI 首次完整执行 `vue-tsc --build` 类型检查（此前本地仅验证过 `vite build`），暴露 4 处历史类型错误（`standalone/handlers.ts` 消息未读计数索引访问、`Signature.vue` 的 `inject` 默认值类型、`TemplateMarket.vue` 的组件反序列化类型断言与评分接口空值判断、`packages/sse-client/ai.ts` 的 AI 润色结果字段兜底），均已修复并本地复现验证通过。最终部署成功，预览地址 https://rpk16v.github.io/FormForge/
