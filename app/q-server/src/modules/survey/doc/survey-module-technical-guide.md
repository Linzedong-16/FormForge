...
- 缓存击穿防护（`cache.ts` 分布式互斥锁）
- 事务内加载组件（`update/publish/close` 消除冗余 DB 查询）
- 路由参数校验统一 `parseAndRespond`（`publish/close` 路由）
...
### 6.2.1 缓存击穿防护（v1.1 新增）

当缓存 Key 过期瞬间，多个并发请求可能同时回源 DB（缓存击穿）。`getOrSet()` 通过 Redis `SET NX` 实现分布式互斥锁：

```
缓存过期瞬间（高并发）：
  ┌──────────────────────────────────────────────┐
  │  请求 1  ──→ get(miss) ──→ SET NX(lock) ✓    │
  │             │ 获取锁 ──→ factory() → set()   │
  │             └─→ del(lock) ──→ 返回数据        │
  │                                              │
  │  请求 2  ──→ get(miss) ──→ SET NX(lock) ✗    │
  │  请求 3      等待 200ms ──→ get(hit) ──→ 返回 │
  │  ...        （缓存已被请求 1 回填）            │
  └──────────────────────────────────────────────┘
```
...
| 场景                | 优化前            | 优化后       |
| ------------------- | ----------------- | ------------ |
| 缓存过期 + 100 并发 | 100 次 DB 查询    | 1 次 DB 查询 |
| Redis 不可用        | 每次查 DB（不变） | 降级查 DB    |
...
| **事务内返回** | `update/publish/close` 在事务内加载组件           | 消除事务后额外 `getById` 查询 |
