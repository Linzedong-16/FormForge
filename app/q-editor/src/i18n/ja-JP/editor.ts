export default {
  // Editor title
  pageTitle: "アンケートエディター",

  // Top action buttons
  updateSurvey: "アンケート更新",
  resetSurvey: "アンケートリセット",
  saveSurvey: "アンケート保存",
  preview: "プレビュー",
  undo: "元に戻す",
  redo: "やり直す",

  // Confirm dialogs
  confirmReset: "アンケートをリセットしますか？",
  confirmTitle: "確認",
  confirm: "確定",
  cancel: "キャンセル",
  confirmButton: "確定",
  cancelButton: "キャンセル",

  // Messages
  resetSuccess: "リセット成功",
  resetCancelled: "リセット取消",
  saveSuccess: "アンケート保存完了",
  saveFailed: "アンケート保存失敗",
  updateSuccess: "アンケート更新完了",
  updateFailed: "アンケート更新失敗",
  saveCancelled: "保存取消",
  deleteSuccess: "削除成功",
  deleteCancelled: "削除取消",

  // Save dialog
  savePromptTitle: "アンケートタイトルを入力してください",

  // Preview dialog
  previewConfirm: "プレビューは自動保存されます。プレビューに移動しますか？",
  previewCancelled: "プレビュー取消",

  // 未保存の変更
  unsavedTitle: "未保存の変更",
  unsavedMessage: "未保存の変更があります。保存してから移動しますか？",
  saveAndLeave: "保存して移動",
  leaveWithoutSave: "破棄",

  // Left panel
  outlineTitle: "概要",
  surveyTypeTitle: "問題タイプ",
  outline: "概要",
  surveyType: "問題タイプ",
  templateMarket: "テンプレート市場",
  addQuestion: "質問を追加してください",

  // AI 機能
  aiPolish: "AIリライト",
  aiGenerate: "AI生成",
  aiInputPlaceholder: "要件を入力してください...",
  aiSubmit: "送信",
  aiClear: "クリア",

  // テンプレート市場
  applyShareTemplate: "共有テンプレート申請",

  // Question type groups
  selectGroup: "選択",
  advancedGroup: "高度",
  inputGroup: "入力",
  noteGroup: "注記",
  personalInfoGroup: "個人情報",
  contactGroup: "連絡先",

  // Selection types
  singleSelect: "単一選択",
  singlePicSelect: "画像単一選択",
  multiSelect: "複数選択",
  multiPicSelect: "画像複数選択",
  optionSelect: "ドロップダウン",

  // Advanced types
  dateTime: "日時",
  rateScore: "評価",
  cascader: "カスケード",
  matrixSingle: "マトリックス単一",
  slider: "スライダー",
  transfer: "並べ替え",

  // Input types
  textInput: "テキスト入力",

  // Note types
  textNote: "テキスト注記",

  // Personal info types
  personalName: "名前",
  personalGender: "性別",
  personalEducation: "学歴",
  personalAge: "年齢",
  personalCareer: "職業",
  personalSchool: "学校",
  personalMajor: "専攻",
  personalIndustry: "業界",
  personalCompany: "会社",
  personalPosition: "役職",
  personalId: "ID番号",

  // Contact types
  personalAddress: "住所",
  personalTel: "電話",
  personalWechat: "WeChat",
  personalQQ: "QQ",
  personalEmail: "メール",

  // Right edit panel
  editPanelTitle: "プロパティエディター",
  clickToEdit: "問題タイプをクリックして編集",
  selectComponentFirst: "画像をアップロードする前に画像質問コンポーネントを選択してください",
  deleteConfirm: "このコンポーネントを削除しますか？",
  deleteTitle: "確認",
  keepTwoOptions: "少なくとも2つのオプションを保持",
  keepTwoItems: "少なくとも2つの項目を保持",
  titleEdit: "タイトル",
  descEdit: "説明",
  optionsEdit: "オプション",
  requiredEdit: "必須",
  positionEdit: "位置",
  sizeEdit: "サイズ",
  weightEdit: "重み",
  italicEdit: "斜体",
  colorEdit: "色",
  textTypeEdit: "テキストタイプ",
  textInputTypeEdit: "入力タイプ",
  dateTimeTypeEdit: "日付タイプ",
  rateTextEdit: "評価テキスト",
  sliderConfigEdit: "スライダー設定",
  matrixOptionsEdit: "マトリックスオプション",
  cascaderOptionsEdit: "カスケードオプション",
  picOptionsEdit: "画像オプション",

  // Option editing
  addOption: "オプション追加",
  removeOption: "オプション削除",
  optionLabel: "オプション",

  // Slider config
  sliderMin: "最小値",
  sliderMax: "最大値",
  sliderStep: "ステップ",
  sliderDefault: "デフォルト",

  // Rating config
  rateMax: "最大評価",
  rateAllowHalf: "半星許可",

  // Date config
  dateFormat: "日付形式",
  timeFormat: "時間形式",
  dateRange: "日付範囲",

  // Cascade config
  cascaderLevels: "カスケードレベル",
  cascaderLevel: "レベル",

  // Matrix config
  matrixRows: "行",
  matrixCols: "列",
  matrixRow: "行",
  matrixCol: "列"
};
